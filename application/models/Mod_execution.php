<?php
class Mod_execution extends CI_Model {

	public function get_count(){	

		$this->db->select(array("o.*"));
		$this->db->from("offers o");
        $this->db->join("users u","u.id=o.manager_id","left");
        $this->db->join("employees_working_hours ewh","o.id=ewh.site_id","left");
        $this->db->where('o.offer_type','execution');
        $this->db->group_by('o.id');
		$query = $this->db->get();
		return $query->num_rows();	
	}
	public function get_executed_offers($id=0){
		//print_r($_POST);
		$column_order = array("o.reference_no","o.description","o.company","o.market","o.client","","o.sale_price - (SELECT SUM(period_bill) FROM erp_site_progress WHERE site_id=o.id)","SEC_TO_TIME(SUM(TIME_TO_SEC(ewh.total_accountable_hours)))"); //set column field database for datatable orderable
		$column_search = array("o.reference_no","o.description","o.company","o.market","o.client"); //set column field database for datatable searchable 
		$order = array('o.id' => 'asc'); // default order 
		
		$this->db->select(array("o.*","DATE(o.created_at) as created_at","IF(o.manager_id>0,u.name,'') as site_manager","(SELECT SUM(period_bill) FROM erp_site_progress WHERE site_id=o.id) as total_bill","SEC_TO_TIME(SUM(TIME_TO_SEC(ewh.total_accountable_hours))) as total_worked_hours"));
		$this->db->from("offers o");	
        $this->db->join("users u","u.id=o.manager_id","left");
        $this->db->join("employees_working_hours ewh","o.id=ewh.site_id","left");
        $this->db->where('o.offer_type','execution');
    ...
    function end_site($id){
       ...
       if($query){
        $compeleted_site =$this->db->select(array('reference_no','description'))->where(array('id'=>$id))->get('offers')->row();
         ...
         $real_purchase = $this->get_exact_purchase($compeleted_site->reference_no);
        #-------------update real purchase price------------------#
        $this->db->update('offers',array('purchase_price'=>$real_purchase), array("id"=>$id));
        #-------------------------------------------------------------------------------#
             $notification = array(
