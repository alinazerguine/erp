...
<html>
<head>
<title>Electrolite | Change Password</title>
				...
				<div class="graphs">
			      ...
			      if ($this->session->flashdata('ok_message')) {
			        ?>
			          ...
			          ?>
							<form method="post" action="<?php echo SURL.'login/update_password'?>">
							<input type="hidden" name="ref_link" value="<?php echo $ref_link;?>">	
							<div class="log-input">
								<div class="log-input-left">
								   <input type="password" class="user" id="password" name="password" required="required" pattern="^\S{6,}$" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Must have at least 6 characters' : ''); if(this.checkValidity()) form.confirm_password.pattern = this.value;" placeholder="Password"/>
								</div>
							...
							<div class="log-input">
								<div class="log-input-left">
								   <input type="password" class="user" id="confirm_password" name="confirm_password" required="required" pattern="^\S{6,}$" onchange="this.setCustomValidity(this.validity.patternMismatch ? 'Password does not match.' : '');" placeholder="Confirm Password" />
								</div>
